...
class Toast {
	...
	domInit(){
		...
		if($(`#toast-${this.options.placement}`).length == 0){
			$('body').append(`
				<div id="toast-${this.options.placement}" class="toast-container position-fixed ${this.placement_class}"></div>
			`);
		}
	}
...
// Helper function for quick toast creation
function toast(title, subtitle, message, options) {
	var t = new Toast(title, subtitle, message, options);
	t.show();
}
